...
@tf.function
def togglemapping2d(
    x, st_element, strides=(1, 1), padding="same", rates=(1, 1), steps=5
):
    """
...
def geodesic_dilation(X, steps=None):
    ...
    _, rec, _ = tf.while_loop(
        condition_equal, update_dilation, [X[0], rec, X[1]], maximum_iterations=steps
    )
    return rec
...
def geodesic_erosion(X, steps=None):
    ...
    _, rec, _ = tf.while_loop(
        condition_equal, update_erosion, [X[0], rec, X[1]], maximum_iterations=steps
    )
...
def distance_step(X):
    ...
    """
    # perform a geodesic dilation with X[0] as marker, and X[1] as mask
    Z = tf.keras.layers.MaxPooling2D(pool_size=(3, 3), strides=(1, 1), padding="same")(
        X[0]
    )
    return [Z, Z + X[1]]
...
def morphological_distance(X, steps=None):
    ...
    new = X
    _, D = tf.while_loop(
        condition_nonzero, update_distance, [new, count], maximum_iterations=steps
    )
...
def leveling(X, steps=None):
    ...
    _, lev, _ = tf.while_loop(
        condition_equal, update_leveling, [X[0], lev, X[1]], maximum_iterations=steps
    )
    return lev
...
class Dilation2D(Layer):
    ...
    def call(self, x):
        ...
        for i in range(self.num_filters):
            ...
            res.append(
                tf.reduce_sum(
                    dilation2d(x, self.kernel[..., i], self.strides, self.padding),
                    axis=-1,
                )
            )
        output = tf.stack(res, axis=-1)
...
class Antidilation2D(Layer):
    ...
    def call(self, x):
        for i in range(self.num_filters):
            out = antidilation2d(
                x, self.kernel[..., i], self.strides, self.padding, self.rates
            )
            if i == 0:
                outputs = out
            else:
...
class TopHatOpening2D(Layer):
    ...
    def call(self, x):
        for i in range(self.num_filters):
            out = self.tophatopening2d(
                x, self.kernel[..., i], self.strides, self.padding
            )
            if i == 0:
                outputs = out
            else:
...
class DepthwiseDilationLines2D(Layer):
    ...
    def build(self, input_shape):
        ...
        self.mask = tf.Variable(
            initial_value=get_lines(self.kernel_size[0]), trainable=False
        )
        if input_shape[self.channel_axis] is None:
            raise ValueError(
                "The channel dimension of the inputs should be defined. Found `None`."
...
class MorphoAverage2D(Layer):
    ...
    def emd2d(self, x, st_element, strides, padding, rates=(1, 1, 1, 1)):
        x1 = tf.nn.dilation2d(
            x, st_element, (1,) + strides + (1,), padding.upper(), "NHWC", rates
        )
        x2 = tf.nn.erosion2d(
            x, st_element, (1,) + strides + (1,), padding.upper(), "NHWC", rates
        )
...
class MinPooling2D(Layer):
    ...
    def __init__(
        self, pool_size, strides, padding="valid", data_format=None, name=None, **kwargs
    ):
...
class MixedPooling2D(Layer):
    ...
    def __init__(
        ...
        self.alpha = tf.Variable(
            initial_value=tf.ones(shape=(1,), dtype="float32") / 2, trainable=True
        )
...
class LasryLionsDE(Layer):
    ...
    def call(self, x):
        ...
        for i in range(self.num_filters):
            y = self.dilation2d(
                x, kernel_c[..., i], self.strides, self.padding, self.rates
            )
            out = self.erosion2d(
                y, kernel[..., i], self.strides, self.padding, self.rates_bis
            )
            if i == 0:
                outputs = out
            else:
...
def MorphoEMP2DShare(input_layer, num_filters, kernel_size, strides):
    ...
    xc = MorphoAverage2D(
        num_filters, kernel_size=kernel_size, strides=strides, padding="valid"
    )(input_layer)
    return xP - xc
...
def MorphoEMD2DQuadratic(input_layer, num_filters, kernel_size, strides):
    ...
    xd = QuadraticDilation2D(
        num_filters, kernel_size=kernel_size, strides=strides, padding="valid"
    )(input_layer)
    xe = QuadraticDilation2D(
        num_filters, kernel_size=kernel_size, strides=strides, padding="valid"
    )(-input_layer)
    xc = (xd - xe) / 2
    return xP - xc
